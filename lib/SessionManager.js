...
function SessionManager() {
    ...
    var createNewSessionObject = function (uid, ip) {
        return new Promise(function (fulfill, reject) {
            var SO = {
                sid: utils.getGibberish(Configuration.get('session.sid_length'),
                    Configuration.get('session.sid_length'),
                    {"special": false, "extendedUpper": false, "extendedLower": false}
                ),
                uid: uid,
                ip: ip,
                seed: utils.getGibberish(Configuration.get('session.seed_length_min'), Configuration.get('session.seed_length_max')),
                timestamp: Date.now(),
    ...
    this.updateSessionObject = function (RO) {
        return new Promise(function (fulfill, reject) {
            ...
            RO.session.seed = utils.getGibberish(Configuration.get('session.seed_length_min'), Configuration.get('session.seed_length_max'));
    ...
    var startGarbageCollector = function () {
        ...
        }, Configuration.get('session.garbage_collection_interval'));
