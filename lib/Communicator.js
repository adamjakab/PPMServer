...
function Communicator() {
        ...
        return new Promise(function(fulfill, reject) {
            decryptRawRequestData_SESSION(RO).then(function() {
                ...
                decryptRawRequestData_USER(RO).then(function() {
                    ...
                    return reject(new CustomError("Decryption failed!", 401));
                });
            });
        });
    };
    ...
    var decryptRawRequestData_SESSION = function(RO) {
        return new Promise(function(fulfill, reject) {
            ...
            filter["timestamp"] = {$gt: Date.now() - Configuration.get('session.lifetime')};
            ...
            UserManager.getAllUsers().then(function(USERS) {
                if(USERS.length) {
                    for(var i = 0; i < USERS.length; i++) {
                        var decrypted = utils.decryptLoginRequest(RO, USERS[i]);
                        if(decrypted !== false) {
    ...
    var getRawRequestData = function(RO, request) {
        return new Promise(function(fulfill, reject) {
            request.on('data', function (data) {
                if ((RO.rawPost.length + data.length) < Configuration.get('communicator.max_allowed_post_length')) {
                    RO.rawPost += data;
                } else {
                    RO.rawPost = false;
                    return reject(new CustomError("HTTP REQUEST LENGTH EXCEEDS ALLOWED LENGTH: " + Configuration.get('communicator.max_allowed_post_length'), 413));
                }
            });
            request.on('end', function () {
                if (RO.rawPost === false || _.isEmpty(RO.rawPost)) {
                    return reject(new CustomError("Unable to get raw request data!", 406));
                } else {
                    utils.log("RAW POST: " + JSON.stringify(RO.rawPost));
                    fulfill();
                }
            });
        });
    ...
    var checkRequestValidity = function(RO, request) {
        return new Promise(function(fulfill, reject) {
            ...
            if (urlParts.pathname != '/') {
                return reject(new CustomError("INVALID PATH: " + urlParts.pathname, 404));
            }

            //CHECK FOR INVALID REQUEST METHODS (only POST is allowed)
            if (request.method != 'POST') {
                return reject(new CustomError("INVALID METHOD: " + request.method, 405));
            }
            ...
            if(Object.keys(urlParts.query).length) {
                return reject(new CustomError("UN-ALLOWED URL PARAMS: " + JSON.stringify(urlParts.query), 414));
            }
            fulfill();
        });
    };
