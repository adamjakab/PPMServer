...
var MAX_LOGIN_POST_LENGTH = 4096;
    ...
    var getRawRequestData = function(RO, request) {
        return new Promise(function(fulfill, reject) {
            request.on('data', function (data) {
                if ((RO.rawPost.length + data.length) < MAX_LOGIN_POST_LENGTH) {
                    RO.rawPost += data;
                } else {
                    RO.rawPost = false;
                    return reject(new CustomError("HTTP REQUEST LENGTH EXCEEDS ALLOWED LENGTH: " + MAX_LOGIN_POST_LENGTH, 413));
                }
            });
