    ...
    var getRawRequestData = function(RO, request) {
        return new Promise(function(fulfill, reject) {
            request.on('data', function (data) {
                if ((RO.rawPost.length + data.length) < Configuration.get('communicator.max_login_post_length')) {
                    RO.rawPost += data;
                } else {
                    RO.rawPost = false;
                    return reject(new CustomError("HTTP REQUEST LENGTH EXCEEDS ALLOWED LENGTH: " + Configuration.get('communicator.max_login_post_length'), 413));
                }
            });
